...
export function BaseBlade({ children, widthClass, size = 'large', className, ...props }: BaseBladeProps) {
    ...
    return (
        <div className={cn("flex flex-col h-full bg-[var(--th-panel-bg)] text-[var(--th-text-primary)] shadow-2xl border-l border-[var(--th-border)] ml-auto overflow-hidden", resolvedWidth, className)} {...props}>
            {children}
        </div>
    );
}
        ...
        <div className="sdui-blade-header sticky top-0 bg-[var(--th-panel-bg)]/90 backdrop-blur z-10 border-b border-[var(--th-border)] min-h-16 px-5 shrink-0 flex items-center justify-between">
            <div className="sdui-blade-title flex items-center gap-3">
                ...
                <div className="sdui-blade-title-text-group flex flex-col">
                    <h2 className="sdui-blade-title-text text-lg font-bold tracking-tight text-[var(--th-text-primary)] leading-tight">{title}</h2>
                    {subtitle && <p className="sdui-blade-subtitle-text text-xs text-[var(--th-text-secondary)] mt-0.5">{subtitle}</p>}
                </div>
            </div>
...
export function BladeLoadingOverlay({ isLoading, text }: { isLoading: boolean; text?: string }) {
    ...
    return (
        <div className="sdui-blade-loading absolute inset-0 z-50 bg-[var(--th-panel-bg)]/60 backdrop-blur-sm flex flex-col items-center justify-center">
            <svg className="animate-spin h-8 w-8 text-[var(--th-text-primary)] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            {text && <span className="sdui-blade-loading-text text-sm font-medium text-[var(--th-text-primary)]">{text}</span>}
        </div>
    );
}
